...
class poseDetector():
    def __init__(self, mode=False, upBody=False, smooth=True,
        ...
        self.upBody = upBody  # track only upper body if true
        self.smooth = smooth  # enable landmark smoothing
        ...
        self.mpPose = mp.solutions.pose  # pose estimation model
    ...
    def findPosition(self, img, draw=True):
        ...
        if self.results.pose_landmarks:
            for id, lm in enumerate(self.results.pose_landmarks.landmark):
                ...
                cx, cy = int(lm.x * w), int(lm.y * h)  # get coordinates
                self.lmList.append([id, cx, cy])
                if draw:
                    cv2.circle(img, (cx, cy), 5, (255, 0, 0), cv2.FILLED)  # draw landmarks
        return self.lmList
